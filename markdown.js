	...
	for(var key in markReg){
		switch (key) {
			...
			case 'reg_font':
				...
				break;
			case 'reg_chart':
				str=str.replace(markReg[key],function(match,p1){
					charts.push(p1.replace(/&gt;/g,'>').replace(/&lt;/g,'<').replace(/&quot;/g,'"').replace(/&apos;/g,"'").replace(/&#47;/g,'/').replace(/&#61;/g,'=').replace(/&amp;/g,'&'));
					htmlArr.push('<div class="chart" id="chart-'+charts.length+'"></div>');
					return ch+(htmlArr.length-1)+ch;
				});
				break;
		}
...
function markdown(text,contianer){
	var result=markdown_replace(text,1);
	contianer.innerHTML=result[0];
	renderlatex();
	for(var i=1;i<=result[1].length;i++){
		Highcharts.chart('chart-'+i,JSON.parse(result[1][i-1]));
	}
}